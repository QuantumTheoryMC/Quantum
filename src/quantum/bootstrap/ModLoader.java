...
enum ModLoader {
	...
	public static List<ModManifest> load(Path directory) throws IOException, ClassNotFoundException, IllegalAccessException {
		System.out.println("[ModLoader] Loading mods...");
		List<ModManifest> loadedMods = new ArrayList<>(directory.getNameCount());
		File[] jarFiles = directory.toFile()
		                           .listFiles((file) -> file.isFile() && file.getName()
		                                                                     .endsWith(".jar"));
		int mods = 0;
		...
		if (jarFiles != null) {
			for (File jarFile : jarFiles) {
				...
				if ((modClass = attributes.getValue("Mod-Class")) != null && !attributes
						                                                              .containsKey("Main-Class") && !attributes
								                                                                                             .containsKey("tweak-Class"))
					try {
						mod = (Mod) (cl = load(jarFile)).loadClass(modClass)
						                                .newInstance();
